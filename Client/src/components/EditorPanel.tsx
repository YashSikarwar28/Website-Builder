//To make changes in the website generated like bg color, padding, margin etc.
...
const EditorPanel = ({
  ...
  const [values, Setvalues] = useState(selectedElements);

  useEffect(() => {
    Setvalues(selectedElements);
  }, [selectedElements]);

  if (!selectedElements || !values) {
    return null;
  }
      ...
      <div className="space-y-4 text-black">
        <div>
          ...
          <textarea
            ...
            className="w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-indigo-400 outline-none min-h-20"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-indigo-400 outline-none"
          />
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 border border-gray-400 rounded-md p-1">
              <input
                ...
                className="w-6 h-6 cursor-pointer"
              />
              <span className="text-xs text-gray-600 truncate">{values.styles.color}
              </span>
            </div>
          </div>
        </div>
